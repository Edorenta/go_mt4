...
async function init_captcha() {
  ...
  var rnx = null;
  var rny = null;
  // while ((rnx < (max_x * 5/100)) || ((rnx > max_x * 95/100))) {
...
async function init_logo() {
  logo_scale_factor = 0.45 * (win.landscape ? win.h : win.w) / 100;

  svg_bar_getter = SVG('logo_div').size(100,100);
  svg_P_getter = SVG('logo_div').size(100,100);
  svg_P = svg_P_getter.svg(
  ...
  //`).id('svg_P');
  `<path d="M25.7 97.1l-.4-36.04h24.8q10 0 0 0c17.3 0 35.7-4.7 35.1-30.23C85.3 9.21 68.8.3 50.3 0H14.9c-.76-.03-1.5.26-2.04.78-.55.53-.86 1.26-.86 2.02v8.01h39.1c5.7.2 11.5 1.5 16 5.61 3.87 3.73 5.98 8.93 5.8 14.31 0 5.51-1.7 10.91-5.8 14.41-4.7 4.01-11 5.01-16.8 5.01H14.8c-1.55 0-2.8 1.25-2.8 2.8V97.1c.05 1.61 1.38 2.9 3 2.9h7.8c1.6 0 2.9-1.3 2.9-3v.1z"/>`).id('svg_P');
  //svg_P.cx = 50;
  //svg_P.cy = 50;
...
//  svg_P.animate(2000, '<', 1000).attr({ fill: '#000' })
  init_core_tw();
  await sleep(1500);
  svg_bar = svg_bar_getter.svg(
  `<linearGradient id="a" x1=".246" y1="-.574" x2=".313" y2="1.019" gradientTransform="matrix(77.19 0 0 89.3 12 11)" gradientUnits="userSpaceOnUse"><stop offset="0%" stop-opacity="0" stop-color="#fff"/><stop offset="79.946%"/></linearGradient><path d="M72.8 97.9L12 11h14.5l62.2 86.9c1 1.3.4 2.4-1.2 2.4H77.4c-1.6 0-3.7-1-4.6-2.4z" fill="url(#a)"/>`).id('svg_bar');
...
*/
  await sleep(600);
  svg_pulse(svg_P, logo_scale_factor);
  svg_pulse(svg_bar, logo_scale_factor);
}
...
async function init_cookie_policy_tw() {
  var tw2 = new TypeWriter("cookie_policy", 15, "▌");
  await tw2.Type("*By proceeding you agree to the site's cookie policy.")
  // await sleep(2500);
  // await tw2.Flush();
