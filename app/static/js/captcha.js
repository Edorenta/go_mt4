...
function init() {
  ...
  let access = QueryParams.get("access");
  let req = QueryParams.get("req");
  if (access == "denied") {
    let banner = new Banner({ text: "Please accept the site's privacy policy to access the " + req + " page." });
  }
  init_logo();
  // component_test = GetComponent("log_in");
}
