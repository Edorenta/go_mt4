...
class ClientSocket
{
    ...
    public:
        ...
        ~ClientSocket();
        bool SendStr(string str_msg);
        bool Send(uchar &bytes[]);
...
bool ClientSocket::Send(uchar &bytes[])
{
    if (!mConnected) return false;
    
    bool bRetval = true;
    int	szToSend = ArraySize(bytes) - 1;
    while (szToSend > 0) {
        int res;
        if (TerminalInfoInteger(TERMINAL_X64)) {
            res = send(mSocket64, bytes, szToSend, 0);
        } else {
            res = send(mSocket32, bytes, szToSend, 0);
        }
        ...
        if (res == SOCKET_ERROR || res == 0) {
            ...
            szToSend -= res;
            if (szToSend > 0) ArrayCopy(bytes, bytes, 0, res, szToSend);
        }
    }
    return bRetval;
}

bool ClientSocket::SendStr(string str_msg)
{
    if (!mConnected) return false;
    
    uchar bytes[];
    bool bRetval = true;
    StringToCharArray(str_msg, bytes);
    int	szToSend = ArraySize(bytes) - 1;
    // Print("bytes to send: " + IntegerToString(szToSend));
    while (szToSend > 0) {
        int res;
        if (TerminalInfoInteger(TERMINAL_X64)) {
            res = send(mSocket64, bytes, szToSend, 0);
...
//retry connecting until established or max attempt is reached
ClientSocket    *socket_connect(ClientSocket *oldClient, string uri,
                                ushort tcp_port, ushort max_attempt) {
    ushort attempt = 0;
    ClientSocket *newClient = NULL;
    static int try = 0;

    delete oldClient;
    while (!(newClient != NULL && newClient.IsSocketConnected())) {
        delete newClient;
        if ((++attempt > max_attempt) && (++try > 0)) {
            Print("Error: Connection to " + uri + ":" + IntegerToString(tcp_port)
                    + " failed [try #" + (string)try + "]");
            Sleep(2000);
            return (NULL);
        }
        newClient = new ClientSocket(uri, tcp_port);
    }
    try = 0;
    Print("Successfully connected to " + uri + ":" + IntegerToString(tcp_port));
    return (newClient);
}
